...
public class MiniGame1 : MonoBehaviour {
	...
	public void Setup(int n){
		...
		// InitializeValues();
		// PaintWhiteSquares();
		// tSelected = GetComponentsInChildren<Node>(true)[nNodes+3].GetComponent<Toggle>();
		// tSelected.isOn = true;
	}
	
	public void SetTitles(){
		List<Toggle> toggles = containVertical.GetComponentsInChildren<Toggle>().ToList();
		char i='A';
		int j = 0;
		foreach (Toggle toggle in toggles){
			// toggle.interactable = j != 0 && toggle.interactable;
			// if ( (j <= nNodes && j>0) || ( j%(nNodes) ==0 && j>nNodes) ){
